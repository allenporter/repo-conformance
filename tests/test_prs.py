...
MOCK_PRS_PAYLOAD = [
    ...
    # 5. Cruft update PR (Ready CI, self-authored valid cruft PR)
    {
        "number": 301,
...
def test_prs_renovate_filter_strictly_excludes_self_authored_and_cruft_with_fake(
    ...
    # Use FakeGitHubClient with real-world PR scenario payload
...
def test_prs_renovate_allow_major_includes_major_bump_with_fake(
    ...
    with patch("builtins.print") as mock_print:
        ...
        # Major bump PR #102 SHOULD be included when allow_major=True
        assert any("#102" in line for line in printed_lines)
        assert any("#101" in line for line in printed_lines)
